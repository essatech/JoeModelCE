...
tbeta_rnd <- function(mn_est, sd_est, low.limit = 0, up.limit = 1) {
  ...
  truncateddist_rtbeta <- function(n, alpha, beta, a = 0, b = 1)
  {
  ...
  for(i in 1:length(mn_est)){
    if (sd_est[i]==0|is.na(sd_est[i])){  #no variance in response
      ...
      if(!(all(all(f_beta[2] > 0) & all(f_beta[1] > 0)))) {
        ...
        rnd.est[i] <- truncateddist_rtbeta(1,
...
sys.cap.func <- function(f.dose.df,
  ...
  rtnorm_TruncatedDistributions <- function (n, mean = 0, sd = 1, a = -Inf, b = Inf)
  {
  ...
  for (i in 1:n.dose) {
    ...
    if (f.dose.df$Distribution[i] == "lognormal") {
      ...
      if (f.dose.df$SD[i]==0){
        ...
        rnd.dose.mat[i,] <- rtnorm_TruncatedDistributions(n.sims,f.dose.df$Mean[i],f.dose.df$SD[i],
                                 a=f.dose.df$Low_Limit[i],b=f.dose.df$Up_Limit[i])
      }
    }
  ...
  return(list(sys.cap=sys.capacity,
              dose=rnd.dose,
