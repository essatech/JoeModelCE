#' Population model across multiple HUCs
#'
...
pop_model_multi_huc <- function (dir_in,
                                      dir_out,
                                      dose_file = NA,
                                      sr_wb_dat = NA,
...
#########################      JUST FOR TESTING. Delete Afterwards    ##########################################################################
  xx = PopulationModel_Batch_Run(
    dir_in = NA,
    dir_out = NA,
    dose = NA,
    sr_wb_dat = NA,
    life_cycle_params = NA,
    HUC_ID = c(1701010201, 1701010202, 1701010203),
    n_years = 100,
    MC_sims = 10,
    stressors = NA,
    output_type = "full"
  ...
  packages <-
    c("ggplot2", "openxlsx", "dplyr", "tidyverse", "lubridate")
  ...
  ASSO = lapply(
    HUC_ID,
    JoeModelCE::PopulationModel_Run,
    dose = dose,
    sr_wb_dat = sr_wb_dat,
  ...
  for (i in 1:length(HUC_ID)) {
    ...
    colnames(HUC_Df) = c("year", paste(
      rep("Stage", MC_sims * 4),
      rep(seq(1:4), MC_sims),
      rep(c("baseline", "ce"), each = MC_sims),
      rep("sim", MC_sims * 4 * 2),
      rep(rep(1:MC_sims, each = 4), 2),
      sep = "_"
    ))
    excel_list[[i]] = HUC_Df
    ...
    colnames(lambda.all) = c(
      "year",
      paste(rep("lambda_baseline", MC_sims), "sim", 1:MC_sims, sep = "_"),
      paste(rep("lambda_ce", MC_sims), "sim", 1:MC_sims, sep = "_")
    )
    ...
    if (output_type == "adult") {
      openxlsx::write.xlsx(
        excel_list[[i]][, c(1, seq(5, (4 * MC_sims + 1), by = 4))],
        file = paste(
          dir_out,
          "/HUC",
          HUC_ID[i],
