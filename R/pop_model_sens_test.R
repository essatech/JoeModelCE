#' Population Model Sensitivity Test
#'
...
pop_model_sens_test <-
  function(dir_in = NA,
    ...
    dir_in = ifelse(is.na(dir_in), dirname(
      system.file("extdata", "stressor_response_fixed_ARTR.xlsx", package = "JoeModelCE")
    ), dir_in)
    dir_out = ifelse(is.na(dir_out), dirname(
      system.file("extdata", "stressor_response_fixed_ARTR.xlsx", package = "JoeModelCE")
    ), dir_out)
    #stress-magnitude workbook
    if (is.na(dose_file)) {
      warning(
        "You need to specify the stressor_magnitude Excel file. The sensitivity analysis will proceed using the default file that came with the package"
      )
      ...
      dose <-
        StressorMagnitudeWorkbook(filename = paste(dir_in, dose_file, sep = ""),
                                  scenario_worksheet = "natural_unc")
    } else if (file.exists(paste(dir_in, dose_file, sep = "/"))) {
      dose <-
        StressorMagnitudeWorkbook(
          filename = paste(dir_in, dose_file, sep = "/"),
          scenario_worksheet = "natural_unc"
        )
    } else{
      stop(
        "Your file is not located inside the proper directory or it is not formated properly to run in the JOEMODEL"
      )
    }
    ...
    if (is.na(sr_wb_dat)) {
      warning(
        "You need to specify the stressor_response Excel file. The sensitivity analysis will proceed using the default file that came with the package"
      )
      ...
      sr_wb_dat <-
        StressorResponseWorkbook(filename = paste(dir_in, sr_wb_dat, sep = ""))
    } else if (file.exists(paste(dir_in, sr_wb_dat, sep = "/"))) {
      sr_wb_dat <-
        StressorResponseWorkbook(filename = paste(dir_in, sr_wb_dat, sep = "/"))
    } else{
    ...
    if (is.na(life_cycle_params)) {
      warning(
        "You need to specify the life cycle parameters Excel file. The sensitivity analysis will proceed using the default file that came with the package"
      )
      ...
      life_cycle_params <-
        read.csv(paste(dir_in, life_cycle_params, sep = ""))
    } else if (file.exists(paste(dir_in, life_cycle_params, sep = "/"))) {
      filename_lc = paste(dir_in, life_cycle_params, sep = "/")
      life_cycle_params <-
        read.csv(paste(dir_in, life_cycle_params, sep = "/"))
    ...
    file.copy(
      ...
      to = paste(
        dir_out,
        "/",
        substr(basename(filename_lc), 1, nchar(basename(filename_lc)) - 4),
        "_sens_analysis.csv",
        sep = ""
      ),
      overwrite = T
    )
    sens_file = read.csv(paste(
      dir_out,
      "//",
      substr(basename(filename_lc), 1, nchar(basename(filename_lc)) - 4),
      "_sens_analysis.csv",
    ...
    if (sum(is.na(sens_param)) > 0) {
      ...
      stop(
        "You specified one or more wrong parameters to assess their sensitivity. Allowed parameters include: k,events,eps,int,SE,S0,SR,surv_1,surv_2,surv_3,surv_4,year_1,year_2,year_3,year_4,cr_E,cr_0,cr_1,cr_2,cr_3,cr_4,mat_1,mat_2,mat_3,mat_4,eps_sd,egg_rho,M.cv,M.rho"
      )
    }
      ...
      if (sum(dose$HUC_ID == huc_ID) == 0) {
        ...
        if (!is.na(stressors)) {
          ...
          sr_wb_dat$main_sheet <-
            sr_wb_dat$main_sheet[which(sr_wb_dat$main_sheet$Stressors %in%
                                         stressors),]
          sr_wb_dat$stressor_names <-
            sr_wb_dat$stressor_names[sr_wb_dat$stressor_names %in%
                                       stressors]
          sr_wb_dat$sr_dat <-
            sr_wb_dat$sr_dat[which(names(sr_wb_dat$sr_dat) %in%
                                     stressors)]
        } else{
          ce_df_sub = ce_df_sub
        }
        ...
        dobj <- jm$sc.dose.df
        merge_cols <-
          ce_df_sub[, c("Stressor", "Life_stages", "Parameters",
                        "Stressor_cat")]
        ...
        colnames(perc.change.baseline) = c("Parameters", paste(rep("Sens_analysis_baseline"), seq(1:MC_sims), sep =
                                                                 "_"))
        ...
        colnames(perc.change.ce) = c("Parameters", paste(rep("Sens_analysis_CE"), seq(1:MC_sims), sep =
                                                           "_"))
        ...
        for (j in 1:sum(!is.na(sens_file$Value.sa))) {
          ...
          life_cycle_params_sens$Value[which(life_cycle_params_sens$Name == sens_i)] =
            sens_file$Value.sa[which(sens_file$Name == sens_i)]
          #life_cycle_params_sens$Value[which(life_cycle_params_sens$Name==sens_i)]=1000
          pop_mod_setup.sens <-
            JoeModelCE::pop_model_setup(life_cycles = life_cycle_params_sens)
          #Generate the matrix for the "SENS" model
          pop_mod_mat.sens <-
            JoeModelCE::pop_model_matrix_elements(pop_mod_setup = pop_mod_setup.sens)
          life_histories.sens <- pop_mod_mat.sens$life_histories
          life_stages_symbolic.sens <- pop_mod_mat.sens$life_stages_symbolic
          density_stage_symbolic.sens <-
            pop_mod_mat.sens$density_stage_symbolic
          ...
          for (ii in 1:MC_sims) {
            ...
            run_with_ce.sens <-
              JoeModelCE::Projection_DD(
                M.mx = life_stages_symbolic.sens,
                D.mx = density_stage_symbolic.sens,
                H.mx = NULL,
                dat = life_histories.sens,
                K = life_histories.sens$Ka,
                Nyears = test_n_years,
                p.cat = 0,
                CE_df = CE_df_rep
              )
            ...
            perc.change.baseline[j, ii + 1] = 100 * (
              median(run_with_baseline_stable.sens) - median(run_with_baseline_stable)
            ) / median(run_with_baseline_stable)
            perc.change.ce[j, ii + 1]       = 100 * (median(pop_ce_stable.sens) -
                                                       median(pop_ce_stable)) / median(pop_ce_stable)
          } #close the loop for all MC simulations
        } #close the loop for all parameters
        test1 = left_join(sens_file,
                          perc.change.baseline,
                          by = c("Parameters" = "Parameters"))
        ...
        reconstruct.sens = data.frame(
          Paremeters = test2$Parameters,
          Name = test2$Name,
          Value = test2$Value,
          Value.sa = test2$Value.sa,
          Median_sensitivity_perc_change_baseline = median_base,
          Median_sensitivity_perc_change_CE = median_CE
        )
