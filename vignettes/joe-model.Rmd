...
6. **Function** Can be either ```'continuous'``` or ```'step'```...
7. **Life_stages** Can be either ```'fry_parr'```, ```'alevin'```, ```'sub_adult'``` or ```'adult'```,...
8. **Parameters** Can be blank or NA (default), which implies the relationship is for system capacity or linked to a vital rate ```'survival'```, ```'capacity'```...
...
1. **HUC_ID** Unique (numeric) identifer for a location of interest. Formally from Hydrological Unit Code (HUC) representing unique subbasins, but can be anything...
...
4. **Stressor_cat** Name of the Stressor category. Note that this must match the spelling used in the stressor response workbook...
